...
public class UIManager : MonoBehaviour
{
    ...
    /// <summary>
    /// LocationManager의 현재 위치를 자동으로 선택
    /// </summary>
    public void SelectCurrentLocation()
    {
        if (mainPanel == null || LocationManager.Instance == null) return;

        var currentLocation = LocationManager.Instance.NowLocation;
        if (currentLocation != null)
        {
            mainPanel.DisplayCurrentLocation();
            Debug.Log($"[UIManager] 현재 위치 기반 UI 표시: {currentLocation.locationName}");
        }
    }
